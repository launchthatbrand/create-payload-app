...
export const Localizer = props => {
  ...
  if (localization) {
    ...
    const t2 = t1.join(" ");
    let t3;
    if ($[2] !== i18n || $[3] !== locale.code || $[4] !== locales || $[5] !== router || $[6] !== setLocaleIsLoading || $[7] !== startRouteTransition || $[8] !== t2) {
      let t4;
      if ($[10] !== i18n || $[11] !== locale.code || $[12] !== locales || $[13] !== router || $[14] !== setLocaleIsLoading || $[15] !== startRouteTransition) {
        t4 = t5 => {
          const {
            close
          } = t5;
